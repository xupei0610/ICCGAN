			...
			case CONTROL_MODE_STABLE_PD:
			{
				...
				if (rbdModel)
				{
					...
					for (int link = 0; link < mb->getNumLinks(); link++)
					{
						...
						{
							// ICCGAN added
							double norm = out_tau[torqueIndex]*out_tau[torqueIndex] + out_tau[torqueIndex+1]*out_tau[torqueIndex+1] + out_tau[torqueIndex+2]*out_tau[torqueIndex+2];
							norm = std::sqrt(norm);
							if (norm > maxForce[torqueIndex]) norm = maxForce[torqueIndex] / norm;
							else norm = 0;
							// ICCGAN added END
						...
						if (dofCount == 1)
						{
							...
							// ICCGAN added
							if (torque > maxForce[torqueIndex])
								torque = maxForce[torqueIndex];
							else if (torque < -maxForce[torqueIndex])
								torque = -maxForce[torqueIndex];
							// ICCGAN added END
